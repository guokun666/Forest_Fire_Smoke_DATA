...
def dota2Darknet(imgpath, txtpath, dstpath, extractclassname):
    ...
    for fullname in filelist:  # fullname='/.../P000?.txt'
        ...
        img_fullname = os.path.join(imgpath, name + '.jpg')  # img_fullname='/.../P000?.jpg'
...
def dota2LongSideFormat(imgpath, txtpath, dstpath, extractclassname):
    if os.path.exists(dstpath):
        shutil.rmtree(dstpath)  # 删除现有的保存的数据
    os.makedirs(dstpath)  # 创建文件夹
    filelist = util.GetFileFromThisRootDir(txtpath)  # 获取dotalabel文件夹中所以的txt 格式为：['/.../P0005.txt', ..., /.../P000?.txt]
    for fullname in filelist:  #遍历所有的txt文件
        objects = util.parse_dota_poly(fullname) #解析txt文件，得到的结果如下
        '''
        objects =
        [{'name': 'smoke', 
          'difficult': '0', 
        ...
        '''
        name = os.path.splitext(os.path.basename(fullname))[0]  # txt文件的名字
        img_fullname = os.path.join(imgpath, name + '.jpg')  # 该txt对应的图片
        img = Image.open(img_fullname)
        img_w, img_h = img.size
        # print img_w,img_h
        with open(os.path.join(dstpath, name + '.txt'), 'w') as f_out:
...
def drawLongsideFormatimg(imgpath, txtpath, dstpath, extractclassname, thickness=2):
    ...
    for fullname in filelist:  # fullname='/.../P000?.txt'
        ...
        name = os.path.splitext(os.path.basename(fullname))[0]  # name='P000?'
        img_fullname = os.path.join(imgpath, name + '.jpg')  # img_fullname='/.../P000?.jpg'
        img_savename = os.path.join(dstpath, name + '_.jpg')  # img_fullname='/.../_P000?.jpg'
...
if __name__ == '__main__':
    dota2LongSideFormat('data/images', #annotaions 文件夹中的标记对应的图片文件夹
                        'data/dotalabel', #2_voc_annotations.py转化后得到的txt文件存放的文件夹
                        'data/yololabels', #最终我们的转化输出的文件夹
                        util.wordname_15) #开始转化

    drawLongsideFormatimg(imgpath='data/images',
                          txtpath='data/yololabels',
                          dstpath='data/longside_img', #可视化转化后的图片保存 文件夹
